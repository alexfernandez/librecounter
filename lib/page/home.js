...
<img src="https://librecounter.org/counter.svg" referrerPolicy="unsafe-url" />
		...
		<p>
		That's it!
		Your stats will start to be collected with ever page view.
		...
		<p>
		There are <a href="styles">more styles available</a>.
		</p>
...
function createRows(labels, data) {
	...
	for (let index = 0; index < labels.length; index++) {
		...
		const row = `		<tr>
			<td>
				${label == '…rest' ? label : `<a href="/${labels[index]}/show">${labels[index]}</a>`}
			</td>
			<td>${data[index]}</td>
		</tr>`
		rows.push(row)
