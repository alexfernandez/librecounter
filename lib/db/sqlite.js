...
export async function upsertOne(name, query, value) {
	...
	const placeholders = keys.map(key => `@${key}`)
	const insert = db.prepare(`INSERT INTO ${name} (${keys.join(', ')}) VALUES (${placeholders.join(', ')}) ON CONFLICT(${keys.join(', ')}) DO UPDATE SET ${value}=${value}+1;`)
	await insert.run(query, value)
}
